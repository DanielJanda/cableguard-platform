...
- **Consequences:** Pin `av==13.1.0` in detector requirements; Control Center launches `pyav_rtsp` + `source_mode=mediamtx` by default; heartbeat `details_json.video_input` carries backend health (no credentials). **Preferred production input** after automated Zahrádky soak + MediaMTX recovery PASS: camera → MediaMTX logical path → localhost RTSP → PyAV → detector. Direct PyAV remains diagnostic/emergency only; OpenCV remains diagnostic fallback. PTS/DTS are stream timeline, not camera wall-clock. On-site visual commissioning is a post-merge ops checklist, not a software merge gate.
- **Status:** ACCEPTED FOR SOFTWARE INTEGRATION (office `.63` visual PASS; Zahrádky automated soak/recovery PASS). Zahrádky on-site visual commissioning pending — not a merge blocker; see `docs/operations/zahradky-onsite-commissioning-checklist.md`.
